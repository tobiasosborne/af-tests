...
## ⚠️ CRITICAL: ℝ-Algebra Refactoring (2026-01-24)

**STATUS**: Refactoring from `FreeAlgebra ℂ` to `FreeAlgebra ℝ` in progress.

**REASON**: Mathlib's star structure on `FreeAlgebra ℂ X` does NOT conjugate scalars:
- `star (algebraMap c) = algebraMap c` (scalars FIXED, not conjugated)
- This breaks scalar extraction: `star(i·1) * (i·1) = -1`, not `|i|² = 1`

**SOLUTION**: Work over ℝ. States are ℝ-linear on self-adjoints, extended to ℂ.
This matches the natural language proof (Section 5.3.5.1).

**TRACKING**: See issues RF-1 through RF-6 (af-tests-zpmh → af-tests-6r38)

---

## Overview

Formalization of the theorem: **A ∈ M̄ ⟺ A is nonnegative in all constrained *-representations**
...
| Weak-* topology | `Topology.Algebra.Module.WeakDual` | `WeakDual 𝕜 E` |
| Seminorms | `Analysis.Seminorm` | `Seminorm 𝕜 E` |
...
Uses FreeAlgebra ℝ (Fin n) - working over ℝ to get correct star behavior.
```

**Key definitions:**
- `FreeStarAlgebra (n : ℕ) := FreeAlgebra ℝ (Fin n)` - Free *-algebra over ℝ
- `generator (j : Fin n)` - The j-th generator gⱼ
- `isSelfAdjoint_generator` - star(gⱼ) = gⱼ

**Mathlib deps:** `Algebra.FreeAlgebra`, `Algebra.Star.Free`

**NOTE**: Working over ℝ means star(c·1) = c·1 for c ∈ ℝ, which is correct!
For c ∈ ℝ, c² = |c|², so scalar extraction gives nonnegative values.

#### File 1.2: `QuadraticModule.lean` (~50 LOC)
```
The quadratic module M = {Σ aᵢ*aᵢ + Σ bⱼₖ*gⱼbⱼₖ}
```

**Key definitions:**
- `QuadraticModule (A : Type*) [StarRing A] (gens : Fin n → A)` - The cone M
- `mem_quadraticModule` - Membership characterization
- `quadraticModule_add_mem` - Closed under addition
- `quadraticModule_smul_mem` - Closed under ≥0 scalar multiplication

**Mathlib deps:** `Geometry.Convex.Cone.Basic`

#### File 1.3: `Archimedean.lean` (~40 LOC)
```
The Archimedean property: ∀a, ∃N, N·1 - a*a ∈ M
```

**Key definitions:**
- `IsArchimedean (M : ConvexCone ℝ A)` - The property
- `archimedean_bound (a : A)` - Witness N for element a

#### File 1.4: `MPositiveState.lean` (~80 LOC)
```
M-positive states: ℝ-linear on self-adjoints, with φ(1)=1, φ(m)≥0 for m∈M
```

**Key definitions:**
- `MPositiveState (n : ℕ)` - Structure with ℝ-linear functional on (A₀)_sa
- `MPositiveStateSet` - The set S_M of all M-positive states
- `apply_m_nonneg` - φ(m) ≥ 0 for m ∈ M
- `toComplex` - Extension to ℂ-valued via φ(a) = ψ(Re a) + i·ψ(Im a)

**Mathlib deps:** `Analysis.Normed.Module.Dual`, `Algebra.Star.SelfAdjoint`

**NOTE**: States are ℝ-linear on self-adjoints, matching the proof in Section 5.3.5.1.
Complex extension is derived, not primitive.
...
```
|φ(a)|² ≤ φ(a*a) ≤ Nₐ for M-positive states
```

**Key lemmas:**
- `cauchy_schwarz_mpositive` - |φ(a)|² ≤ φ(a*a) (uses GNS CS)
- `archimedean_bound_mpositive` - φ(a*a) ≤ Nₐ
- `eval_bounded` - |φ(a)| ≤ √Nₐ
...
**Key lemmas:**
- `selfAdjoint_decomp` - x = ¼(1+x)*(1+x) - ¼(1-x)*(1-x)
...
```
Equip (A₀)* with pointwise convergence topology
```
...
**Key lemmas:**
- `state_space_bounded` - S_M ⊆ ∏ Disk(√Nₐ)
- `state_space_closed` - S_M is closed (intersection of preimages)
- `state_space_compact` - Tychonoff + closed ⟹ compact
...
- `stateSeminorm_finite` - ||a||_M ≤ √Nₐ < ∞

**Mathlib deps:** `Analysis.Seminorm`

#### File 4.2: `QuadraticModuleClosure.lean` (~30 LOC)
```
M̄ = closure of M in ||·||_M topology
```

**Key definitions:**
- `quadraticModuleClosure` - M̄
- `closure_is_cone` - M̄ is a convex cone
- `kernel_in_closure` - N ⊆ M̄ where N = {a : ||a||_M = 0}
...
**Key lemmas:**
- `cstar_le_state_seminorm` - ||a||_{C*} ≤ ||a||_M
- `state_le_cstar_seminorm` - ||a||_M ≤ ||a||_{C*} (uses GNS)
...
```
If A ∉ M̄, construct ψ₀ on span{A} with ψ₀(A) < 0
```

**Key lemmas:**
- `span_cap_M_trivial` - M ∩ span{A} = {0} when A ∉ M̄
- `separating_on_span` - Define ψ₀(λA) = -λε

#### File 5.3: `RieszExtension.lean` (~40 LOC)
```
Extend ψ₀ to ψ on all of (A₀)_sa using Riesz-Kantorovich
```

**Key lemmas:**
- `extend_to_selfadjoint` - ψ : (A₀)_sa → ℝ with ψ ≥ 0 on M ∩ (A₀)_sa
- `extension_negative_at_A` - ψ(A) = -ε < 0

**Mathlib deps:** `Analysis.Convex.Cone.Extension` (`riesz_extension`)

#### File 5.4: `DualBackward.lean` (~40 LOC)
```
Complete the (⟸) direction and combine
```

**Key lemmas:**
- `complex_extension` - Extend ψ to φ : A₀ → ℂ
- `normalize_state` - φ₁ = φ/ψ(1) ∈ S_M
- `contradiction` - φ₁(A) < 0 contradicts hypothesis
...
```
Constrained *-representation: π with π(gⱼ) ≥ 0
```
...
**Key lemmas:**
- `gns_rep_generator_positive` - π_φ(gⱼ) ≥ 0
- `gns_rep_bounded` - ||π_φ(a)|| ≤ √Nₐ
...
**Key theorem:**
```lean
theorem main_theorem (A : (A₀)_sa) :
    A ∈ quadraticModuleClosure ↔
    ∀ π : ConstrainedStarRep, 0 ≤ π A
```
...
```
S_M ≠ ∅ via scalar extraction functional
```

**Key lemmas:**
- `scalar_extraction` - φ₀(a) = coefficient of empty word
- `scalar_extraction_mpositive` - φ₀ ∈ S_M
