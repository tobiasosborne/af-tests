...
**H-O approach (§3.1)**: Use functional calculus on C(a), the subalgebra generated by a.
- C(a) is associative, commutative (power-associativity), finite-dimensional
- C(a) is reduced (no nilpotents, by formal reality)
- By Artinian structure theorem: C(a) ≅ ℝⁿ (product of copies of ℝ)
- The coordinate idempotents in ℝⁿ give the spectral projections
- Apply `spectral_decomp_of_eigenvector_csoi` with these idempotents
