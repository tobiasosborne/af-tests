...
- Open blockers:
  ...
  - Long one-argument boundary branches remain: e.g.
    `Eq258X k (xCons i (yCons m r')) one` and its y/left-one mirrors. These are
    harder than the exact rearrangement lemmas because they need the full `k` split,
    (2.49)/(2.47), property (iii), and lower-weight family facts.
...
1. Prove/wrap the remaining long boundary cases generated by `M_op` where one
   argument is `one` and the other is a long alternating monomial:
   `Eq258X k (xCons i (yCons m r')) one`,
   `Eq258X k one (yCons j (xCons m r'))`, and the y-symmetric pair.
