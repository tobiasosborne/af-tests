...
**af-s4t7 continues**: The spectral decomposition chain

Remaining sorries in SpectralTheorem.lean:
1. `spectral_projection_exists` (line 99) - construct idempotent from eigenspace
2. `spectral_decomposition_exists` (line 113) - main existence theorem

**Key insight discovered**: The challenge is showing eigenspaces contain idempotents. In matrix
algebras, eigenspaces of L_A contain the spectral projections. The approach should be:
1. For each eigenspace E_μ, find an idempotent e_μ ∈ J such that P₁(e_μ) ⊆ E_μ
2. Show these idempotents form a CSOI
3. Apply `spectral_decomp_of_eigenvector_csoi`

**Alternative approach**: Use H-O's functional calculus on C(a) (subalgebra generated by a).
C(a) is associative, reduced, Artinian → product of copies of ℝ → natural idempotents.
