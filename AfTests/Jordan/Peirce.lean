...
**WARNING**: PeirceSpace e 1 is NOT associative for general idempotents!

**Counterexample**: For e = diag(1,1,0) in H₃(ℝ), PeirceSpace e 1 ≅ H₂(ℝ)
(2×2 symmetric matrices), which is NOT associative under Jordan product.

H-O 2.5.5 says "elements that operator-commute with p generate associative
subalgebras" - this means the algebra generated by a SINGLE element a and p
is associative (power associativity), NOT that the entire PeirceSpace p 1
is associative.

For PRIMITIVE idempotents p, H-O 2.9.4(ii) shows {pAp} = ℝp, which is
trivially associative (1-dimensional). The ring structure approach for
general idempotents is invalid.
-/
