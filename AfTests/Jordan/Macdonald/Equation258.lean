...
This is a narrower package suggested by the completed H-O case split:
ordinary Eq(2.58) facts are only needed under the actual side conditions
`p ∈ X, q ∈ Y` and `p ∈ Y, q ∈ X`. For raw-right facts this only records the
safe boundary shapes currently known to be true. The cross raw-right facts
generated by the `<` branches need a separate adapter because some total-syntax
right-start same-letter cases are not H-O theorem instances. -/
structure Eq258DriverWFCore (n : ℕ) : Prop where
  ...
  rawXBoundary :
    ∀ (k : ℕ) (p : FreeAssocMono),
      p.inY → p.WF → p.weight < n → Eq258XRawRight k p one
  rawYBoundary :
    ∀ (j : ℕ) (p : FreeAssocMono),
      p.inX → p.WF → p.weight < n → Eq258YRawRight j p one
