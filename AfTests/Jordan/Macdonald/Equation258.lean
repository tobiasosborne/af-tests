...
/-- Well-formed driver core for the eventual recursive proof.

This is the narrower package suggested by the completed H-O case split:
ordinary Eq(2.58) facts are only needed under the actual side conditions
`p ∈ X, q ∈ Y` and `p ∈ Y, q ∈ X`, while raw-right facts are only needed in
the cross orientations generated by the `<` branches. This avoids making the
final induction prove arbitrary total-syntax raw-right statements that are not
part of H-O's theorem. -/
structure Eq258DriverWFCore (n : ℕ) : Prop where
  xWF :
    ∀ (k : ℕ) (p q : FreeAssocMono),
      p.inX → q.inY → p.WF → q.WF → p.weight + q.weight < n → Eq258X k p q
  yWF :
    ∀ (j : ℕ) (p q : FreeAssocMono),
      p.inY → q.inX → p.WF → q.WF → p.weight + q.weight < n → Eq258Y j p q
  rawXCross :
    ∀ (k : ℕ) (p q : FreeAssocMono),
      p.inY → q.inX → p.WF → q.WF → p.weight + q.weight < n → Eq258XRawRight k p q
  rawYCross :
    ∀ (j : ℕ) (p q : FreeAssocMono),
      p.inX → q.inY → p.WF → q.WF → p.weight + q.weight < n → Eq258YRawRight j p q

/-- The earlier broad total-syntax package trivially supplies the narrower
    well-formed core. The final driver should aim for the core first, then only
    widen if a concrete recursive call requires it. -/
theorem Eq258DriverWFCore.of_driverIH {n : ℕ} (hIH : Eq258DriverIH n) :
    Eq258DriverWFCore n where
  xWF := fun k p q _ _ _ _ hlt => hIH.x k p q hlt
  yWF := fun j p q _ _ _ _ hlt => hIH.y j p q hlt
  rawXCross := fun k p q _ _ _ _ hlt => hIH.rawRight k p q hlt
  rawYCross := fun j p q _ _ _ _ hlt => hIH.rawRightY j p q hlt
...
/-! ### Driver-ready raw-right weight≤1 cases -/
