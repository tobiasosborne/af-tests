...
theorem primitive_peirce_one_dim_one [FinDimJordanAlgebra J] [FormallyRealJordan J]
  ...
  have hle_span : (PeirceSpace e 1 : Submodule ℝ J) ≤ Submodule.span ℝ {e} := by
    ...
    -- The proof uses that the subalgebra generated by x (with identity e) decomposes
    -- as a product of fields, each of which must be ℝ by formal reality.
    -- By primitivity, there can be only one field factor, so x ∈ ℝ·e.
    -- Key steps:
    -- 1. Powers of x stay in P₁(e) by jpow_succ_mem_peirce_one
    -- 2. By finite dimension, x satisfies a polynomial
    -- 3. Formal reality eliminates complex roots and repeated roots
    -- 4. Primitivity eliminates multiple distinct roots
    -- 5. Hence x = λe for some λ
    sorry
