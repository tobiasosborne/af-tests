...
theorem primitive_peirce_one_dim_one [FinDimJordanAlgebra J] [FormallyRealJordan J]
  ...
  have hle_span : (PeirceSpace e 1 : Submodule ℝ J) ≤ Submodule.span ℝ {e} := by
    ...
    -- Strategy: Discriminant analysis on a quadratic relation.
    -- Step A: Get quadratic relation x² = r•x + s•e from finite dimension
    --   (Since powers stay in P₁(e) and P₁(e) is finite-dim, x satisfies a polynomial)
    -- Step B: Discriminant Δ = r² + 4s case analysis:
    --   Case Δ ≤ 0: Use peirce_one_sq_nonpos_imp_zero with y = x - (r/2)•e
    --   Case Δ > 0: Use lagrange_idempotent_in_peirce_one to get idempotent f in P₁(e),
    --              then by primitive_idempotent_in_P1, f = 0 or f = e, giving x ∈ ℝ•e
    -- TODO: Prove quadratic relation exists from finite dimension
    -- The algebraic computation in lagrange_idempotent_in_peirce_one also needs completion
    sorry
