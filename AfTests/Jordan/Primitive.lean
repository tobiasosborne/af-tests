...
/-- If x ∈ P₁(e) satisfies x² = r•x + s•e with discriminant Δ = r² + 4s > 0,
then the Lagrange element f = (1/√Δ)•(x - μ•e) (where μ = (r-√Δ)/2) is an idempotent
in P₁(e). By primitivity, f = 0 or f = e, giving x ∈ ℝ•e. -/
theorem lagrange_idempotent_in_peirce_one [FormallyRealJordan J]
    {e : J} (he : IsIdempotent e) {x : J} (hx : x ∈ PeirceSpace e 1)
    {r s : ℝ} (hquad : jsq x = r • x + s • e) (hΔ_pos : r^2 + 4*s > 0) :
    let Δ := r^2 + 4*s
    let μ := (r - Real.sqrt Δ) / 2
    let f := (1 / Real.sqrt Δ) • (x - μ • e)
    IsIdempotent f ∧ f ∈ PeirceSpace e 1 := by
  intro Δ μ f
  have hΔ_nonneg : 0 ≤ Δ := le_of_lt hΔ_pos
  have hsqrt_pos : Real.sqrt Δ > 0 := Real.sqrt_pos.mpr hΔ_pos
  ...
  · -- Show f ∈ PeirceSpace e 1: f is a linear combination of x and e, both in P₁(e)
    ...
    have hμe : μ • e ∈ PeirceSpace e 1 := Submodule.smul_mem _ μ he_in
    have hdiff : x - μ • e ∈ PeirceSpace e 1 := Submodule.sub_mem _ hx hμe
...
Proof outline: For any x ∈ P₁(e), powers xⁿ stay in P₁(e). By finite dimension,
x satisfies a polynomial. If this polynomial has multiple distinct roots, we can
construct non-trivial idempotents via Lagrange interpolation, contradicting
primitivity. If it has repeated roots, we get nilpotents, contradicting formal
reality. So x = λe for some λ, hence P₁(e) = ℝ·e. -/
theorem primitive_peirce_one_dim_one [FinDimJordanAlgebra J] [FormallyRealJordan J]
  ...
  have hspan_le : Submodule.span ℝ {e} ≤ PeirceSpace e 1 := by
    ...
    exact he_in
  -- For equality, we show P₁(e) ≤ span{e} by proving every x is a scalar multiple of e
  have hle_span : (PeirceSpace e 1 : Submodule ℝ J) ≤ Submodule.span ℝ {e} := by
    ...
    · exact ⟨0, by simp [hx_zero]⟩
    -- Case 2: x ≠ 0
    -- x² ∈ P₁(e) by closure
    have hx2 : jsq x ∈ PeirceSpace e 1 := by
      rw [← jpow_two]
      exact jpow_succ_mem_peirce_one he.isIdempotent hx 1
    -- Step A: Get quadratic relation x² = r•x + s•e from finite dimension
    -- The subalgebra ℝ[x] = span{e, x, x², ...} is finite-dim (subspace of P₁(e))
    -- and power-associative. By minimal polynomial theory, x satisfies some
    -- polynomial relation. The simplest case is a quadratic.
    -- For now, we extract this as a sub-lemma with sorry.
    obtain ⟨r, s, hquad⟩ : ∃ r s : ℝ, jsq x = r • x + s • e := by
      -- This requires finite dimension argument or minimal polynomial analysis
      -- Key insight: In the power-associative subalgebra generated by x,
      -- either x ∈ ℝ•e directly, or x satisfies a minimal polynomial.
      -- By formal reality (no nilpotents) and primitivity (no non-trivial idempotents),
      -- this polynomial must be linear or quadratic with Δ ≤ 0.
      -- TODO: Formalize this argument using Artinian ring theory or direct analysis
      sorry
    -- Step B: Discriminant Δ = r² + 4*s case analysis
    rcases le_or_gt (r^2 + 4*s) 0 with hΔ_le | hΔ_pos
    · -- Case Δ ≤ 0: y = x - (r/2)•e satisfies y² = (Δ/4)•e with Δ/4 ≤ 0
      let y := x - (r/2) • e
      have hy_in : y ∈ PeirceSpace e 1 := Submodule.sub_mem _ hx (Submodule.smul_mem _ _ he_in)
      -- Compute y²
      have hy_sq : jsq y = ((r^2 + 4*s) / 4) • e := by
        -- y² = (x - (r/2)e)² = x² - r(xe) + (r²/4)e²
        -- Since x ∈ P₁(e): xe = x, and e² = e
        -- y² = x² - rx + (r²/4)e = (rx + se) - rx + (r²/4)e = (s + r²/4)e = ((r² + 4s)/4)e
        have hxe : jmul e x = x := by rw [mem_peirceSpace_iff] at hx; simp [hx]
        have hxe' : jmul x e = x := by rw [jmul_comm, hxe]
        -- Expand y² step by step
        have h1 : jsq y = jsq x - r • x + (r^2/4) • e := by
          simp only [y, jsq_def, sub_jmul, jmul_sub, jmul_smul, smul_jmul, hxe, hxe']
          -- Goal: jmul x x - (r/2)•x - (r/2)•(x - (r/2)•jmul e e) = jmul x x - r•x + (r²/4)•e
          -- Convert jmul e e to e using isIdempotent
          have he_jmul : jmul e e = e := by rw [← jsq_def]; exact he.isIdempotent
          rw [he_jmul]
          -- Goal: jmul x x - (r/2)•x - (r/2)•(x - (r/2)•e) = jmul x x - r•x + (r²/4)•e
          have h : (r / 2) * (r / 2) = r^2 / 4 := by ring
          rw [smul_sub, smul_smul, h]
          -- Goal: jmul x x - (r/2)•x - ((r/2)•x - (r²/4)•e) = jmul x x - r•x + (r²/4)•e
          module
        rw [h1, hquad]
        -- Now: (r • x + s • e) - r • x + (r^2/4) • e = ((r^2 + 4*s)/4) • e
        have h2 : (r • x + s • e) - r • x + (r ^ 2 / 4) • e = (s + r^2/4) • e := by module
        rw [h2]
        congr 1
        ring
      -- Since Δ ≤ 0, we have Δ/4 ≤ 0
      have hcoef_le : (r^2 + 4*s) / 4 ≤ 0 := by linarith
      -- By peirce_one_sq_nonpos_imp_zero, y = 0
      have hy_zero := peirce_one_sq_nonpos_imp_zero he.isIdempotent he.ne_zero hy_in hcoef_le hy_sq
      -- y = 0 means x - (r/2)•e = 0, so x = (r/2)•e
      use r/2
      have h : x - y = (r/2) • e := by simp only [y]; module
      rw [← h, hy_zero, sub_zero]
    · -- Case Δ > 0: Lagrange idempotent construction
      -- By lagrange_idempotent_in_peirce_one, we get an idempotent f ∈ P₁(e)
      ...
      -- In both cases, x is a scalar multiple of e
      let Δ := r^2 + 4*s
      let μ := (r - Real.sqrt Δ) / 2
      let lam := (r + Real.sqrt Δ) / 2  -- λ is reserved keyword
      let f := (1 / Real.sqrt Δ) • (x - μ • e)
      rcases hf_cases with hf_zero | hf_eq_e
      · -- f = 0: (1/√Δ)(x - μe) = 0, so x = μe
        have hf_zero' : f = 0 := hf_zero
        have hsqrt_ne : Real.sqrt Δ ≠ 0 := ne_of_gt (Real.sqrt_pos.mpr hΔ_pos)
        have h : x - μ • e = 0 := by
          have h1 : (1 / Real.sqrt Δ) • (x - μ • e) = 0 := hf_zero'
          rw [smul_eq_zero] at h1
          rcases h1 with h_scalar | h_diff
          · exfalso; simp [hsqrt_ne] at h_scalar
          · exact h_diff
        use μ
        have h2 : μ • e = x - (x - μ • e) := by module
        rw [h2, h, sub_zero]
      · -- f = e: (1/√Δ)(x - μe) = e, so x - μe = √Δ·e, hence x = (μ + √Δ)e = λe
        have hf_eq_e' : f = e := hf_eq_e
        have hsqrt_pos : Real.sqrt Δ > 0 := Real.sqrt_pos.mpr hΔ_pos
        have h : x - μ • e = Real.sqrt Δ • e := by
          have h1 : (1 / Real.sqrt Δ) • (x - μ • e) = e := hf_eq_e'
          have hsqrt_ne : Real.sqrt Δ ≠ 0 := ne_of_gt hsqrt_pos
          have h2 : Real.sqrt Δ • ((1 / Real.sqrt Δ) • (x - μ • e)) = x - μ • e := by
            rw [smul_smul]
            simp only [mul_one_div_cancel hsqrt_ne, one_smul]
          rw [← h2, h1]
        use lam
        have h2 : lam • e = μ • e + Real.sqrt Δ • e := by
          simp only [lam, μ, ← add_smul]
          congr 1
          ring
        rw [h2, ← h]
        module
